import type { DtcCode } from "../src/types/passport.js";
import { describe, it, expect } from "vitest";
import { extractDtc } from "../src/ingest/dekra/extractors.js";
...
describe("extractDtc", () => {
  ...
  it("finds proper OBD-II codes and ignores random words", () => {
    ...
    const out = extractDtc(t);
    expect(out.codes.map((c: DtcCode) => c.code)).toEqual(["P0420", "P0301"]);
  ...
  it("marks red when MIL ON or current", () => {
    ...
    expect(out.codes.map((c: DtcCode) => c.code)).toEqual(["U0100", "P0700"]);
  });
